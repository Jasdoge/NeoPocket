...
// Handles the input pins to save power
void toggleInputPins( bool on = false ){
	...
	if( on ){
		...
		pinMode(PIN_BATTERY_READ_IN, OUTPUT);
		digitalWrite(PIN_BATTERY_READ_IN, LOW);
		
		pinMode(PIN_CHARGE_STAT, OUTPUT);
		digitalWrite(PIN_CHARGE_STAT, HIGH);	// It's pulled up via 10k

		// SDA/SCL are pulled up so this should limit them a bit
		digitalWrite(7, HIGH);
		digitalWrite(6, HIGH);
